...
def addrec():
	if request.method == 'POST':
		...
		here=request.form
		requests.post('https://172.22.0.3:5000/registration',json=dict(here),verify=False)
...
def chkrec():
	if request.method == 'POST':
		...
		password = request.form['password']
		here=request.form
		response = requests.get('https://172.22.0.3:5000/login',json=dict(here),verify=False)
